...
pr.0p <- predict(lr.0, newdata = y, type = "response") # Actual response value
y$error <- 2 # Incorrect Classification
y$error[pr.0 == y$language] <- 3 # Correct Classification
par(mar=c(5.1, 5.1, 5.1, 8.1), xpd=TRUE)
plot(pr.0p, ylab="Prediction",xlab = 'Observation', main="Logistic Regression",
     col = y$error, pch = 16)
legend('right', inset = c(-0.3,0),c('Correct', 'Incorrect'), col = c(3,2), 
       pch = 16)
dev.off() # Reset Graph Options
```
...
models <- c("Logisitc", "Linear SVM", "Gaussian SVM", "Kullback-Leibler SVM")
classerrors <- c(mis.l, mis.svl, mis.svg, mis.svk)
errortable <- data.frame(Model=models, "Misclassification"=classerrors)
kable(errortable)
...
pr.app <- predict(svg.app, newdata = test, type = "response")

# Misclassification rate
1-mean(pr.app == test$language)
```
